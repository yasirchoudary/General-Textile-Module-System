...
import type { AccountHead, Contract, Party, ReceiveVoucher } from "@gtms/types";
...
export function ReceiveForm() {
  ...
  const [recent, setRecent] = useState<ReceiveVoucher[]>([]);
  const [date, setDate] = useState(todayInputValue());
  const [partyId, setPartyId] = useState("");
  const [contractId, setContractId] = useState("");
  const [headId, setHeadId] = useState("head-bank");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [partyList, contractList, headList, vouchers] = await Promise.all([
          mockFinanceApi.listParties(),
          mockFinanceApi.listContracts(),
          mockFinanceApi.listHeads(),
          mockFinanceApi.listReceives(),
        ]);
        if (!active) return;
        setParties(partyList);
        setContracts(contractList);
        const liquidHeads = headList.filter((h) => h.id === "head-cash" || h.id === "head-bank");
        setHeads(liquidHeads.length > 0 ? liquidHeads : headList);
        if (liquidHeads[0]) setHeadId(liquidHeads[0].id);
  ...
  async function onSubmit(event: FormEvent) {
    ...
    setSaving(true);
    try {
      const voucher = await mockFinanceApi.createReceive({
        date,
        partyId,
        contractId,
        headId,
      ...
      setSuccess(`Saved receive voucher ${voucher.voucherNo} for ${formatCurrency(voucher.amount)}.`);
      setDescription("");
      setAmount(0);
      const vouchers = await mockFinanceApi.listReceives();
      setRecent(vouchers.slice(0, 5));
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <FinancePageHeader
        ...
        description="Date · Party · Contract · Account Head · Description · Amount"
      />
      ...
      >
        ...
        <div>
          <Label htmlFor="head">Deposit To (Cash / Bank)</Label>
          <Select
            id="head"
            value={headId}
        ...
        <div>
          ...
          {filteredContracts.length === 0 ? (
            <p className="mt-1 text-xs text-[var(--color-ink-subtle)]">
              No contracts for this party yet.
            </p>
          ) : null}
        </div>
        ...
        <div className="lg:col-span-2">
          ...
          <Textarea
            ...
            placeholder="Payment received narration / invoice settlement"
          />
        </div>
        ...
        <div className="lg:col-span-2">
          <Button type="submit" loading={saving}>
            Save receive voucher
          </Button>
        </div>
      </form>

      <RecentReceiveVouchers rows={recent} parties={parties} contracts={contracts} />
    </div>
  );
}

function RecentReceiveVouchers({
  rows,
  parties,
  contracts,
}: {
